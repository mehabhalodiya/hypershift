...
Memory working set over the last week, per control plane container:
...
!!! note
    ...
    manifests, using the illustrative values above for the Medium and Large rows. Your
    exact totals will differ with platform and enabled capabilities. Actual consumption
    on an idle cluster is well below the requested total, but size the management
    cluster on requests, because requests are what the scheduler enforces.
